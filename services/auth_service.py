...
async def register_user(request: RegisterRequest, background_tasks: BackgroundTasks, db):
    ...
    user = MsUser(email=request.email, password=hashed_password, is_verified=False, verification_token=verification_token)
...
async def verify_email(token: str, db):
    repo = AuthRepository(db)
    user = db.query(MsUser).filter(MsUser.verification_token == token).first()
    if not user:
...
async def reset_password_with_token(request: ResetPasswordTokenRequest, db):
    repo = AuthRepository(db)
    user = db.query(MsUser).filter(MsUser.reset_token == request.token).first()
    if not user:
        raise HTTPException(status_code=400, detail="Invalid token")
